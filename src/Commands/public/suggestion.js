...
module.exports = {
    ...
    execute(interaction) {
        ...
        const failed = new EmbedBuilder().setDescription("There is no channel called 💡suggestions💡 in this server")
        const reply = new EmbedBuilder().setDescription("Your suggestion has been submitted!")

        const sug = new EmbedBuilder()
            .setAuthor({name: interaction.user.tag, iconURL: interaction.user.displayAvatarURL({ dyanmic: true })})
            .setColor('#8a2be2')
